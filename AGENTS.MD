...
The Renderer Layer (`src/renderer`) is fully sandboxed. It has \*\*zero access\*\* to Node.js built-ins (`fs`, `child\_process`, `path`). All system interactions must pass through the strictly typed explicit context bridge exposed via `window.api`.
